...
for i=1:5000
    af=actualforce(nr_agents,am,acclTime,bodyFactor,F,delta,pm);
% walls=[[100, 100, 700, 100]; [100, 700, 100, 100]; [100, 700, 700, 700]; [700, 100, 700, 382]; [700, 700, 700, 418]];
    swf=wallforce(nr_agents,am,acclTime,bodyFactor,F,delta,pm,walls);
    spf=peopleforce(nr_agents,am,acclTime,bodyFactor,F,delta);
...
function spf= peopleforce(nr_agents,am,acclTime,bodyFactor,F,delta)
...
for j=1:nr_agents
    ...
    pf=F.*exp((r_ij-mod_d_ij)/(delta)).*e_ij+bodyFactor.*max(r_ij-mod_d_ij,0).*e_ij+k*max(r_ij-mod_d_ij,0).*dot((am(:,5)-am(:,6)),t_ij).*t_ij;
    pf(isnan(pf))=0;
    pf=pf.*am(:,9);
...
function swf= wallforce(nr_agents,am,acclTime,bodyFactor,F,delta,pm,walls)
...
for i=1:nr_agents
    ...
    r_i=am(i,4);
        for j=1:size(walls)
            ...
            wf=wf+1*(-F.*exp((r_i-d_iw)/(delta)).*e_iw-bodyFactor.*max(r_i-d_iw,0).*e_iw+k*max(r_i-d_iw,0).*dot((am(:,5)-am(:,6)),t_ij).*t_ij);
        end
    swf(i,:)=wf;
end
...
function af = actualforce(nr_agents,am,acclTime,bodyFactor,F,delta,pm)
